...
- **`internal/nwwsclient`**: the connection lifecycle. `handleMessage` (the XMPP router's per-stanza handler) does parse → store → optionally launch PAN async, tracked via a small mutex-based `tracker` (deliberately not `sync.WaitGroup`: the library's per-stanza goroutines call `add()` with no ordering against shutdown's `wait()`, which `WaitGroup` forbids and the race detector flags) so shutdown can drain in-flight PAN scripts. `Run()` owns connect/MUC-join/reconnect-backoff/graceful-shutdown.

Reconnection backoff is 1s/2s/4s/.../60s-capped, resets on successful reconnect. **Every connection attempt, initial or reconnect, builds a brand-new `xmpp.Client`** (see the gotcha below) and is bounded by `connectTimeout` (60s). Each client's error callback is tagged with a generation number so late errors from a dead/abandoned client are ignored rather than tearing down the live one. `Retry=false` skips reconnect entirely and exits non-zero on disconnect. The initial connection attempt is *not* covered by `Retry` — only disconnects after a successful first connect trigger the retry loop.
...
- **Never reuse an `xmpp.Client` across connections.** In v0.5.1 `XMPPTransport.Connect()` never resets the transport's `isSecure` flag and `NewSession` reuses the old `Session` (`TlsEnabled` stays true), so a second `Connect()` on the same client believes the fresh plaintext socket is already encrypted, **skips STARTTLS, and sends SASL credentials in the clear** (or gets rejected/stalls against a stricter server — the symptom seen in production was every reconnect failing with `expected <stream> or <open> but got <iq>` plus a stray `unknown namespace urn:xmpp:ping`, the latter from a background goroutine `connect()` spawns after a failed `NewSession`). `TestRun_ReconnectRenegotiatesTLSAndKeepsReceiving` guards this against a local mock server (`mockserver_test.go`). Also, `Client.Resume()` never restarts `recv()`/`keepalive()` (only `Connect()` does), so it is not an alternative.
- `Client.Connect()` sets no read deadline beyond the initial TCP dial's `ConnectTimeout`; if the server hangs mid-handshake it blocks forever (production saw ~3.5 min per reconnect attempt). `runCancelable()` races it against a timeout/the shutdown context. An abandoned client must only be `Disconnect()`ed *after* its `Connect()` returns — calling it concurrently races on the transport's fields (the `afterAbandoned` callback exists for this).
